...
listener(void *args) {
    ...
    if ((tcpsock = socket(AF_INET, SOCK_RAW, IPPROTO_ICMP)) < 0) {
    ...
    while (true) {
        ...
        if (n > 0) {
            if (FD_ISSET(rcvsock, &rfds)) { // process ICMP packet
                nullreads = 0;
            ...
            if (FD_ISSET(tcpsock, &rfds)) { // process TCP packet
                nullreads = 0;
                memset(buf, 0, PKTSIZE);
                len = recv(tcpsock, buf, PKTSIZE, 0);
                if (len == -1) {
                    cerr << ">> Listener: TCP read error: " << strerror(errno) << endl;
                    continue;
                }
